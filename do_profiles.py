...
def output_profiles(profile_config, nml, out_dir, plot=False):
    ...
    # z_ml = 0 # height of initial mixed layer
    # z_ft = 0 # start of free troposphere (with exponential qt profile)
    # qt_ml = 0.016 # qt in the mixed layer
    # thl_ml = None # thl in the mixed layer

    # # replace wind_high by dudz, wind_low by u0  dudz = 0.0022 m/s/m
    # thls = 298.5 # sea surface thl
    # dthl0 =1.25 # surface thl offset from thls
    # u0 = -3 # surface wind
    # v0 = 3 
    # dudz = 0.0022  # wind shear
    # dvdz = 0.0022

    # w0 = 4e-3   # default 4e-3 m/s = 0.4 cm/s  (positive number for downward motion)
    # wpamp = 0 # sine-shape amplitude in subsidence

    # # Subsidence - exponential profile
    # Hw  = 2500  # m   subsidence height scale (was 1000 for botany-5)
    # Hwp = 5300  # m

    # qt = exp(z, qt0, qt_lambda, z_ml)
    # thl = linmlsurf(z, Gamma, thls, dthl0, z_ml)
    # u = lin(z, u0, dudz)
    # v = lin(z, v0, dudz)
    # # v = np.zeros_like(z)
    # w_subs = expsinw(z, w0, Hw, wpamp, Hwp)
    ...
    # thl_tend_0     = 0        # surface tendency value
    # qt_tend_0      = 0
    # thl_tend_z_max = 0 # z where tendency has dropped to 0
    # qt_tend_z_max  = 4000
